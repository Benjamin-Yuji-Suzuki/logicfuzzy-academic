...
pub fn trapmf(x: f64, a: f64, b: f64, c: f64, d: f64) -> f64 {
    assert!(a <= b && b <= c && c <= d, "trapmf: requires a <= b <= c <= d");
    ...
    // Rampa de subida: [a, b)
    if x >= a && x < b {
        if (b - a).abs() < f64::EPSILON { return 1.0; }
        return (x - a) / (b - a);
    }

    // Rampa de descida: (c, d]
    if x > c && x <= d {
        if (d - c).abs() < f64::EPSILON { return 1.0; }
        return (d - x) / (d - c);
    }
...
pub fn interp_membership(universe: &[f64], memberships: &[f64], value: f64) -> f64 {
    assert_eq!(universe.len(), memberships.len(),
        "interp_membership: universe and memberships must have the same length");

    if universe.is_empty() { return 0.0; }

    let first = universe[0];
    let last  = *universe.last().unwrap();

    if value <= first { return memberships[0]; }
    if value >= last  { return *memberships.last().unwrap(); }

    let pos = universe.partition_point(|&u| u <= value);
    if pos == 0 { return memberships[0]; }
...
mod tests {
    ...
    // trimf
    #[test] fn trimf_pico()         { assert_eq!(trimf(50.0, 0.0, 50.0, 100.0), 1.0); }
    #[test] fn trimf_fora_esq()     { assert_eq!(trimf(-1.0, 0.0, 50.0, 100.0), 0.0); }
    #[test] fn trimf_fora_dir()     { assert_eq!(trimf(101.0, 0.0, 50.0, 100.0), 0.0); }
    #[test] fn trimf_meio_subida()  { assert!((trimf(25.0, 0.0, 50.0, 100.0) - 0.5).abs() < 1e-10); }
    #[test] fn trimf_meio_descida() { assert!((trimf(75.0, 0.0, 50.0, 100.0) - 0.5).abs() < 1e-10); }

    // trapmf
    #[test] fn trapmf_plano()       { assert_eq!(trapmf(50.0, 20.0, 30.0, 70.0, 80.0), 1.0); }
    #[test] fn trapmf_borda_b()     { assert_eq!(trapmf(30.0, 20.0, 30.0, 70.0, 80.0), 1.0); }
    #[test] fn trapmf_borda_c()     { assert_eq!(trapmf(70.0, 20.0, 30.0, 70.0, 80.0), 1.0); }
    #[test] fn trapmf_fora_esq()    { assert_eq!(trapmf(10.0, 20.0, 30.0, 70.0, 80.0), 0.0); }
    #[test] fn trapmf_fora_dir()    { assert_eq!(trapmf(90.0, 20.0, 30.0, 70.0, 80.0), 0.0); }
    #[test] fn trapmf_aberta_esq()  { assert_eq!(trapmf(0.0,  0.0,  0.0,  30.0, 50.0), 1.0); }
    #[test] fn trapmf_aberta_dir()  { assert_eq!(trapmf(100.0,60.0, 80.0,100.0,100.0), 1.0); }

    // gaussmf
    #[test] fn gaussmf_pico()       { assert!((gaussmf(50.0, 50.0, 10.0) - 1.0).abs() < 1e-10); }
    #[test] fn gaussmf_simetria()   {
        assert!((gaussmf(40.0,50.0,10.0) - gaussmf(60.0,50.0,10.0)).abs() < 1e-10);
    }

    // MembershipFn enum
    #[test] fn enum_trimf_eval() {
        let mf = MembershipFn::Trimf([0.0, 50.0, 100.0]);
        assert_eq!(mf.eval(50.0), 1.0);
        assert!((mf.eval(25.0) - 0.5).abs() < 1e-10);
    }
    #[test] fn enum_trapmf_eval() {
        let mf = MembershipFn::Trapmf([0.0, 0.0, 30.0, 50.0]);
        assert_eq!(mf.eval(0.0), 1.0);
        assert_eq!(mf.eval(15.0), 1.0);
    }
    #[test] fn enum_eval_universe() {
        let mf = MembershipFn::Trimf([0.0, 50.0, 100.0]);
