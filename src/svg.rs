...
#[inline] fn px(x: f64, min: f64, max: f64) -> f64 { ML + (x - min) / (max - min) * PW }
#[inline] fn py(d: f64)                            -> f64 { MT + PH - d.clamp(0.0, 1.0) * PH }
#[inline] fn fv(v: f64) -> String {
    if (v - v.round()).abs() < 1e-9 && v.abs() < 10_000.0 { format!("{:.0}", v) }
    else { format!("{:.1}", v) }
}
...
fn line(s: &mut String, x1: f64, y1: f64, x2: f64, y2: f64, stroke: &str, sw: f64, dash: &str) {
    let da = if dash.is_empty() { String::new() } else { format!(r#" stroke-dasharray="{}""#, dash) };
    s.push_str(&format!(
...
#[allow(clippy::too_many_arguments)]
fn text(s: &mut String, x: f64, y: f64, anchor: &str, fill: &str, size: u32, bold: bool, content: &str) {
...
fn circle(s: &mut String, cx: f64, cy: f64, r: f64, fill: &str) {
    s.push_str(&format!(r#"<circle cx="{:.1}" cy="{:.1}" r="{}" fill="{}"/>"#, cx, cy, r, fill));
}
...
fn draw_grid_axes(s: &mut String, var: &FuzzyVariable) {
    ...
    for &d in &[0.25_f64, 0.5, 0.75, 1.0] {
        let y = py(d);
        let col = if (d - 1.0).abs() < 1e-9 { OVERLAY } else { SURFACE };
        line(s, ML, y, ML + PW, y, col, 1.0, "");
        text(s, ML - 6.0, y + 3.5, "end", MUTED, 9, false, &format!("{:.2}", d));
    }
...
fn draw_legend(s: &mut String, var: &FuzzyVariable) {
    ...
    // Title
    text(s, ML + 8.0, ly + 14.0, "start", MUTED, 8, false, "Terms / Termos:");
...
fn draw_intersection(
    ...
    let lx  = if xp + lw + 6.0 > ML + PW { xp - lw - 6.0 } else { xp + 6.0 };
...
pub fn render_variable_svg(var: &FuzzyVariable, input: Option<f64>) -> String {
    ...
    text(&mut s, W / 2.0, 36.0, "middle", MUTED, 9, false,
    ...
    if let Some(val) = input {
        if val >= min && val <= max {
            ...
            s.push_str(&format!(
                ...
                xp, ty + 3.0, xp - 5.0, ty + 11.0, xp + 5.0, ty + 11.0
            ));

            // Input value label
            text(&mut s, xp, ty + 26.0, "middle", YELLOW, 9, true, &format!("x = {}", fv(val)));
...
pub fn render_aggregated_svg(
    ...
    // Title
    text(&mut s, W / 2.0, 22.0, "middle", TEXT, 13, true,
        &format!("{} — aggregated output", var.name));
    text(&mut s, W / 2.0, 36.0, "middle", MUTED, 9, false,
        &format!("Mamdani clip (min) + aggregation (max)  |  centroid = {:.4}", centroid));
    ...
    for (idx, term) in var.terms().iter().enumerate() {
        let color  = PALETTE[idx % PALETTE.len()];
        let firing = firing_degrees.iter()
        ...
        if firing > 1e-9 {
            ...
            text(&mut s, xr, yr, "end", color, 9, true, &format!("α={:.4}", firing));
        }
    ...
    if centroid >= min && centroid <= max {
        ...
        text(&mut s, xp, ty + 26.0, "middle", YELLOW, 9, true,
...
mod tests {
    ...
    #[test] fn svg_starts_and_ends_correctly() {
        let svg = render_variable_svg(&make_var(), None);
        assert!(svg.starts_with("<svg"));
        assert!(svg.ends_with("</svg>"));
    }

    #[test] fn svg_contains_variable_name() {
        assert!(render_variable_svg(&make_var(), None).contains("temperature"));
    }

    #[test] fn svg_contains_all_term_labels() {
        let svg = render_variable_svg(&make_var(), None);
    ...
    #[test] fn svg_contains_legend() {
        let svg = render_variable_svg(&make_var(), None);
        assert!(svg.contains("Terms / Termos:"), "deve ter strip de legenda");
    }

    #[test] fn svg_legend_contains_all_term_labels() {
    ...
    #[test] fn svg_with_input_has_vertical_marker() {
        let svg = render_variable_svg(&make_var(), Some(10.0));
        assert!(svg.contains(r#"stroke-dasharray="5,3""#));
    }

    #[test] fn svg_with_input_has_mu_annotations() {
        let svg = render_variable_svg(&make_var(), Some(10.0));
        assert!(svg.contains("μ_cold"));
    }

    #[test] fn svg_without_input_no_vertical_marker() {
        let svg = render_variable_svg(&make_var(), None);
        assert!(!svg.contains(r#"stroke-dasharray="5,3""#));
    }

    #[test] fn svg_input_outside_universe_ignored() {
        let with_out = render_variable_svg(&make_var(), Some(999.0));
        let without  = render_variable_svg(&make_var(), None);
        assert_eq!(with_out, without, "input fora do universo deve ser ignorado");
    }

    #[test] fn svg_input_at_lower_bound() {
        let svg = render_variable_svg(&make_var(), Some(0.0));
        assert!(svg.contains(r#"stroke-dasharray="5,3""#));
    }

    #[test] fn svg_input_at_upper_bound() {
        let svg = render_variable_svg(&make_var(), Some(50.0));
        assert!(svg.contains(r#"stroke-dasharray="5,3""#));
    }

    #[test] fn svg_trapmf_renders() {
        let mut v = FuzzyVariable::new("x", Universe::new(0.0, 100.0, 1001));
        v.add_term(Term::new("mid", MembershipFn::Trapmf([20.0,30.0,70.0,80.0])));
        let svg = render_variable_svg(&v, Some(50.0));
        assert!(svg.contains("mid"));
        assert!(svg.contains("1.0000")); // μ at plateau
    }

    #[test] fn svg_gaussmf_renders_with_input() {
        let mut v = FuzzyVariable::new("x", Universe::new(0.0, 100.0, 1001));
        v.add_term(Term::new("peak", MembershipFn::Gaussmf { mean: 50.0, sigma: 10.0 }));
        let svg = render_variable_svg(&v, Some(50.0));
        assert!(svg.contains("μ_peak"));
    }

    #[test] fn svg_single_term_no_panic() {
    ...
    #[test] fn svg_eight_terms_all_colors() {
        let mut v = FuzzyVariable::new("x", Universe::new(0.0, 80.0, 101));
        for i in 0..8 {
            v.add_term(Term::new(
                &format!("t{}", i),
                MembershipFn::Trimf([i as f64 * 10.0, i as f64 * 10.0 + 5.0, i as f64 * 10.0 + 10.0])
            ));
        }
        let svg = render_variable_svg(&v, None);
        // Todos os 8 termos aparecem na legenda
        for i in 0..8 { assert!(svg.contains(&format!("t{}", i))); }
    }

    // ── render_aggregated_svg ──────────────────────────────────────

    #[test] fn aggregated_svg_contains_centroid() {
    ...
    #[test] fn aggregated_svg_contains_legend() {
    ...
    #[test] fn aggregated_svg_centroid_outside_universe_no_triangle() {
        let mut v = FuzzyVariable::new("out", Universe::new(0.0, 100.0, 1001));
        v.add_term(Term::new("low", MembershipFn::Trimf([0.0,0.0,50.0])));
        ...
        assert!(svg_in.contains("centroid = 50.0000"),
        ...
        assert_eq!(count_200, 1,
            "200 deve aparecer apenas no subtitulo, nao no marcador; encontrou {} vezes", count_200);
    }

    #[test] fn aggregated_svg_alpha_label_visible() {
        let mut v = FuzzyVariable::new("out", Universe::new(0.0, 100.0, 1001));
