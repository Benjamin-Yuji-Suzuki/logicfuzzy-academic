...
//! use fuzzy_mamdani::rule::{Rule, RuleBuilder, Connector};
...
//!
//! // Regras
//! motor.add_rule(RuleBuilder::new().when("temperatura","fria").then("velocidade","lenta"));
//! motor.add_rule(RuleBuilder::new().when("temperatura","quente").then("velocidade","rapida"));
//!
//! // Calculo
//! motor.set_input("temperatura", 0.0);
//! let resultado = motor.compute();
...
impl MamdaniEngine {
    ...
    pub fn compute(&self) -> HashMap<String, f64> {
        ...
        for regra in &self.rules {
            ...
            let nome_cons = regra.consequent_var();
            let termo_cons = regra.consequent_term();

            let cons_var = match self.consequents.get(nome_cons) {
                Some(v) => v,
                None => continue, // consequente desconhecido: ignorar
            };

            // Avalia a curva de pertinencia do termo consequente sobre
            // todo o universo discretizado
            let curva_mf = cons_var.term_membership_curve(termo_cons);

            // (3) Clip (implicacao de Mamdani): recorta a MF no grau de disparo
            // Equivalente: clipped[i] = min(mf[i], grau)
            //
            // (4) Agregacao simultanea: max entre regras que afetam o mesmo
            // consequente — cada ponto do universo recebe o maior valor
            // entre todas as regras que o afetam
            let agg = agregado.get_mut(nome_cons).unwrap();
            for (i, &mu) in curva_mf.iter().enumerate() {
                let recortado = mu.min(grau); // clip
                if recortado > agg[i] {
                    // max
                    agg[i] = recortado;
                }
        ...
        // ── (5) Defuzzificacao por centroide ──────────────────────
        // Centroide discreto: sum(x[i] * mu[i]) / sum(mu[i])
        // Equivalente a fuzz.defuzz(universe, aggregated, 'centroid')
        ...
        for (nome, agg_mf) in &agregado {
            ...
            let numerador: f64 = pontos
                .iter()
                .zip(agg_mf.iter())
                .map(|(&x, &mu)| x * mu)
                .sum();
            let denominador: f64 = agg_mf.iter().sum();

            let saida = if denominador.abs() < f64::EPSILON {
                // Nenhuma regra disparou: retorna ponto medio do universo
    ...
    pub fn explain(&self) -> ExplainReport {
        ...
        for regra in &self.rules {
            ...
            if let Some(cons_var) = self.consequents.get(nome_cons) {
                let curva_mf = cons_var.term_membership_curve(termo_cons);
                let agg = agregado.get_mut(nome_cons).unwrap();
                for (i, &mu) in curva_mf.iter().enumerate() {
        ...
        // ── Etapa 3: Defuzzificacao por centroide ────────────────────
...
mod tests {
    ...
    fn motor_simples(mf_entrada: MembershipFn, mf_saida: MembershipFn) -> MamdaniEngine {
        ...
        motor.add_rule(RuleBuilder::new().when("x", "a").then("y", "b"));
        motor
    }
    ...
    fn engine_add_regras() {
        ...
        m.add_rule(RuleBuilder::new().when("x", "a").then("y", "b"));
    ...
    fn centroide_mf_uniforme_e_ponto_medio() {
        ...
        m.set_input("x", 5.0); // pleno disparo (grau=1.0)
        let r = m.compute();
    ...
    fn clip_grau_zero_retorna_ponto_medio_do_universo() {
        // Grau=0.0: nenhuma regra dispara → ponto medio gracioso
        let mut m = motor_simples(
            MembershipFn::Trimf([0.0, 0.0, 5.0]), // fora da regiao de x=8
            MembershipFn::Trimf([0.0, 5.0, 10.0]),
        );
        m.set_input("x", 8.0); // grau de "a" em 8.0 = 0.0
        let r = m.compute();
        // Sem disparo → ponto medio do universo [0,10] = 5.0
        assert!((r["y"] - 5.0).abs() < 0.01, "ponto_medio={:.4}", r["y"]);
    }
    ...
    fn clip_desloca_centroide_para_baixo() {
        ...
        m.set_input("x", 4.5); // grau de "a" em 4.5 ≈ 0.5
        let r_clip = m.compute();
    ...
    fn agregacao_duas_regras_nao_sobrepostas() {
        ...
        motor.add_rule(RuleBuilder::new().when("x", "esq").then("y", "baixo"));
        motor.add_rule(RuleBuilder::new().when("x", "esq").then("y", "alto"));

        motor.set_input("x", 0.0); // x=0 → esq=1.0
        let r = motor.compute();
    ...
    fn agregacao_max_entre_regras_concorrentes() {
        ...
        motor.add_rule(RuleBuilder::new().when("x", "tudo").then("y", "baixo"));
        motor.add_rule(RuleBuilder::new().when("x", "tudo").then("y", "alto"));

        motor.set_input("x", 5.0);
        let r = motor.compute();
    ...
    fn ventilador_cenario1_frio_seco_resultado_lento() {
        ...
        m.set_input("temperatura", 5.0);
        m.set_input("umidade", 10.0);
        let r = m.compute();
        assert!(
    ...
    fn ventilador_cenario2_morno_medio_resultado_medio() {
        ...
        let mut m = montar_ventilador();
        m.set_input("temperatura", 25.0);
        m.set_input("umidade", 50.0);
        let r = m.compute();
    ...
    fn ventilador_cenario3_quente_umido_resultado_rapido() {
        ...
        let mut m = montar_ventilador();
        m.set_input("temperatura", 45.0);
        m.set_input("umidade", 90.0);
        let r = m.compute();
        assert!(
    ...
    fn ventilador_monotonia_temperatura() {
        ...
        for &t in &temps {
            let mut m = montar_ventilador();
            m.set_input("temperatura", t);
            m.set_input("umidade", umid);
            let v = m.compute()["velocidade_ventilador"];
            assert!(
                v >= anterior - 0.5,
    ...
    fn explain_output_igual_ao_compute() {
        ...
        m.set_input("temperature", 10.0);
        let compute_val = m.compute()["speed"];
        let explain_val = m.explain().outputs["speed"];
        assert!(
    ...
    fn explain_ventilador_cenario_frio_seco() {
        ...
        let compute_val = m.compute()["velocidade_ventilador"];
        let report = m.explain();
