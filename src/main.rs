...
use fuzzy_mamdani::{antecedent, consequent, export_svg, fuzzy_var, rule, var_svg, MamdaniEngine};
...
fn sistema_gorjeta() {
    ...
    engine.set_input("food_quality", 6.5);
    engine.set_input("service", 5.0);
    ...
    for (desc, q, sv) in scenarios {
        engine.set_input("food_quality", *q);
        engine.set_input("service", *sv);
        let tip = engine.compute()["tip"];
        let cls = if tip < 8.0 {
            "LOW"
        } else if tip < 16.0 {
            "MEDIUM"
...
fn sistema_irrigacao() {
    ...
    for (desc, moist, temp) in scenarios {
        engine.set_input("moisture", *moist);
        engine.set_input("temperature", *temp);
        let v = engine.compute()["valve"];
        let cls = if v < 33.0 {
            "LOW"
        } else if v < 66.0 {
            "MEDIUM"
